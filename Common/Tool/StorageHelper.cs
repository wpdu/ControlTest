...
namespace WinSFA.Common.Utility
{
    public class StorageHelper
    {
        ...
        internal static async Task<Stream> ReadFileStream(string fullPath)
        {
            Stream result = null;
            try
            {
                ...
                result = await local.OpenStreamForReadAsync(fullPath);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("FileNotExist:" + fullPath);
        ...
        #region Read　& Write by Absolute Uri

        internal static async Task<Stream> RadStreamByUri(string uri)
        {
            Stream result = null;
            try
            {
                StorageFile file = await StorageFile.GetFileFromApplicationUriAsync(new Uri(uri, UriKind.Absolute));
                result = await file.OpenStreamForReadAsync();
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("RadStreamByUri:" + uri);
            }
            return result;
        }
        ...
        public const string PackageFolder = "ms-appx:///";
        public const string LocalFolder = "ms-appdata:///local/";

        public static async Task<StorageFile> GetStorageFile(string path, FolderType folderType)
        {
            StorageFile ret = null;
            string folder = PackageFolder;
            if (folderType == FolderType.Local)
            {
                folder = LocalFolder;
            }
            Uri uri = new Uri(string.Format("{0}{1}", folder, path));
            try
            {
                var file = await StorageFile.GetFileFromApplicationUriAsync(uri);
                ret = file;
            ...
            {
                System.Diagnostics.Debug.WriteLine("GetStorageFile no File :" + path);
            }
            return ret;
        }
    }
